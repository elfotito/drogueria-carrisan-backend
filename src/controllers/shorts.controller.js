import axios from 'axios';
import { YOUTUBE_CHANNEL_ID } from '../config/youtube.js';

const PAGE_URL = 'https://www.youtube.com/channel';
const CACHE_TTL_MS = 5 * 60 * 1000;
const MAX_VIDEOS = 12;
const UA = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0 Safari/537.36';

let cache = { videos: null, timestamp: 0 };

export function parsearPagina(html) {
  const data = extraerYtInitialData(html);
  if (!data) return [];

  const tabs = data.contents?.twoColumnBrowseResultsRenderer?.tabs || [];
  const tab = tabs.find((t) => t.tabRenderer?.title === 'Videos')
    || tabs.find((t) => t.tabRenderer?.selected)
    || tabs[0];
  if (!tab?.tabRenderer) return [];

  const contents = tab.tabRenderer.content?.richGridRenderer?.contents || [];
  const videos = [];
  for (const item of contents) {
    const lockup = item?.richItemRenderer?.content?.lockupViewModel;
    if (!lockup) continue;

    const id = lockup.contentId;
    if (typeof id !== 'string' || !/^[\w-]{11}$/.test(id)) continue;

    const title = lockup.metadata?.lockupMetadataViewModel?.title?.content;
    let titulo = '';
    if (typeof title === 'string') titulo = title;
    else if (title?.runs) titulo = title.runs.map((r) => r.text).join('');

    const fuente = lockup.contentImage?.thumbnailViewModel?.image?.sources?.at(-1)?.url;
    const thumb = typeof fuente === 'string'
      ? fuente.split('?')[0]
      : `https://i.ytimg.com/vi/${id}/hq720.jpg`;

    videos.push({ id, titulo: titulo.trim(), thumb, url: `https://www.youtube.com/watch?v=${id}` });
  }
  return videos.slice(0, MAX_VIDEOS);
}

function extraerYtInitialData(html) {
  const marcadores = ['window["ytInitialData"] = ', 'var ytInitialData = '];
  for (const marcador of marcadores) {
    const inicio = html.indexOf(marcador);
    if (inicio === -1) continue;
    const cuerpo = html.slice(inicio + marcador.length);
    let profundidad = 0;
    let enString = false;
    let escapado = false;
    for (let i = 0; i < cuerpo.length; i++) {
      const c = cuerpo[i];
      if (enString) {
        if (escapado) escapado = false;
        else if (c === '\\') escapado = true;
        else if (c === '"') enString = false;
        continue;
      }
      if (c === '"') { enString = true; continue; }
      if (c === '{') profundidad++;
      else if (c === '}') {
        profundidad--;
        if (profundidad === 0) {
          try {
            return JSON.parse(cuerpo.slice(0, i + 1));
          } catch {
            return null;
          }
        }
      }
    }
  }
  return null;
}

export async function getShorts(req, res) {
  if (!YOUTUBE_CHANNEL_ID) {
    return res.json({ videos: [], configurado: false });
  }

  const ahora = Date.now();
  if (cache.videos && ahora - cache.timestamp < CACHE_TTL_MS) {
    return res.json({ videos: cache.videos, configurado: true });
  }

  try {
    const { data: html } = await axios.get(`${PAGE_URL}/${YOUTUBE_CHANNEL_ID}/videos`, {
      headers: { 'User-Agent': UA, 'Accept-Language': 'es' },
      timeout: 15000,
    });
    const videos = parsearPagina(html);
    if (videos.length === 0) {
      console.error('shorts: pagina sin videos parseables para', YOUTUBE_CHANNEL_ID);
      return res.status(502).json({ error: 'No se pudo obtener los videos de YouTube', videos: [] });
    }
    cache = { videos, timestamp: ahora };
    return res.json({ videos, configurado: true });
  } catch (err) {
    console.error('shorts: error obteniendo pagina', err.message);
    if (cache.videos) {
      return res.json({ videos: cache.videos, configurado: true });
    }
    return res.status(502).json({ error: 'No se pudo obtener el feed de YouTube', videos: [] });
  }
}